...
6. JSON 状態をコピーして、同じ盤面をローカルで共有します。

## 設定

MVP にはランタイム設定ファイルはありません。盤面サイズとサンプル名は `src/app.ts` に定義され、純粋なグリッド動作は `src/grid.ts`、`src/algorithms.ts`、`src/samples.ts` にあります。
...
振る舞いテストは、最短経路、BFS 再生フレーム、壁の処理、到達不能な盤面、JSON 往復、壊れた入力の拒否、決定的なサンプル生成をカバーします。
